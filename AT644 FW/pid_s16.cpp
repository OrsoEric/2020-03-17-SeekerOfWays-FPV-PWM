...
**		2020-02-14
**	OrangeBot needs wildly different FP for the position (high FP) and speed (low FP)
**	FP is now a class variable
****************************************************************************/
...
//! @return int16_t |
//!	@details
//!	Set the fixed point position
/***************************************************************************/

uint8_t &Pid_s16::gain_fp( void )
{
	//--------------------------------------------------------------------------
	//	RETURN
	//--------------------------------------------------------------------------
...
//!	check_config | void
/***************************************************************************/
//! @return bool | true = configuraton parameters of PID class are bad
//!	@details
//! Check untime configuration of the PID
/***************************************************************************/

bool Pid_s16::check_config( void )
{
	//Trace Enter
	DENTER();
	...
	if (this -> g_cmd_min >= this -> g_cmd_max)
	{
		DRETURN_ARG("ERR: min %d is above max %d", this -> g_cmd_min, this -> g_cmd_max );
		return true;
	}
	//Check FP gain
	if (this -> g_fp > Pid_s16_config::MAX_GAIN_FP)
	{
		DRETURN_ARG("ERR: FP position too large: %d", this -> g_fp );
		return true;
	}
...
int16_t Pid_s16::exe( int16_t reference, int16_t feedback )
{
	...
	if (this -> g_kp != 0)
	{
		...
		DPRINT("proportional | error: %5d | gain: %5d | command: %5d\n", err, this -> g_kp, (int32_t)err *this -> g_kp /(1<<this -> g_fp));
	}
	...
	if (this -> g_ki != 0)
	{
		...
		cmd32 += acc *this -> g_ki;
		DPRINT("integral     | error: %5d | gain: %5d | command: %5d\n", acc, this -> g_ki, (int32_t)acc *this -> g_ki /(1<<this -> g_fp));
	}
	...
	*/
	...
	cmd32 = SSHR_RTO( cmd32, this -> g_fp );
...
inline void Pid_s16::init( void )
{
	...
	//Initialize floating point position
	this -> g_fp		= (uint8_t)0;
