...
class Raspicam_browser
{
	...
	get_feed()
	{
		var msk = "raspivid 180 -t 0 -o - -w %d -h %d -fps %d -vf -hf";
		var cmd = lib_util.format(msk, this.options.width, this.options.height, this.options.fps);
		
		console.log("NODE | Raspicam_browser : ", cmd);
		
		var streamer = lib_spawn("raspivid", ['-t', '0', '-o', '-', '-w', this.options.width, '-h', this.options.height, '-fps', this.options.fps, '-pf', 'baseline', "-vf", "-hf"]);
