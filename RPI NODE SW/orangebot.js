...
//		2020-02-28
//	turret, speed and fire messages from  .html
//		2020-03-06
//	test servo electronics and tst_servo.JS
//	Calibration of PID
//	Complete test of remote control
...
//		2020-02-28
//	Added key bindings for speed, turret and fire
...
//Time between updates of the servomotors
const time_update_servo_position = 333
...
//The speed of the robot is in ten discrete speed grades
var g_speed_grade = 0;
...
(
	...
	function (socket)
	{
		...
		//Periodically compute servo position from speed
		setInterval
		(
			function()
			{
				//Process servo position
				g_servo = update_servo_pos( g_servo, g_servo_spd );
				//Send servo message
				send_robot_servo_message( g_servo );
			},
			//Send periodically
			time_update_servo_position
		);
		...
		(
			...
			function (data)
			{
				...
				[g_right, g_left] = process_direction( g_pid.ctrl_mode, data, g_speed_grade );
...
function process_direction( ctrl_mode, direction, speed_grade )
{
	...
	switch(ctrl_mode)
	{
		...
		case "pwm":
		{
			...
			velocity = g_pwm_factor *(1 +speed_grade);
		}
		case "slow_spd":
		{
		...
		case "fast_spd":
		{
			...
			{
				//Fetch speed for this mode
				velocity = g_spd_factor *(1 +speed_grade);
			}
			//compute PWM
...
//	TURRET
//-----------------------------------------------------------------------------------
//	Process a turret message coming from the browser
//	Update settings of rotation and elevation servo
//	@todo

function process_turret( data )
{
	//Set speed
	g_servo_spd.rotation	= data.right;
	g_servo_spd.elevation	= data.up;
...
//	SPEED
//-----------------------------------------------------------------------------------
//	Process a speed message coming from the browser
//	Update speed grade

function process_speed( data )
{
	//Update speed grade
	g_speed_grade = data.speed;
...
//The RPI wants to move the three servomotors
function send_robot_servo_message( servo )
{
	var int_rotation = Math.round(servo.rotation); 
	var int_elevation = Math.round(servo.elevation);
	var int_aux = Math.round(servo.aux);
	
	//Add plus signs when needed
	var message = "SERVO_POS" +  int_rotation + ":" + int_elevation + ":" + int_aux + "\0";
	//Send message through UART
	uart_transmit( message );
...
//The RPI wants to fire the main weapon
function send_robot_fire_message( servo )
{
	//Add plus signs when needed
	var message = "FIRE" + "\0";
	//Send message through UART
...
//Integrate turret servo position
function update_servo_pos( servo_pos, servo_spd )
{
	const max_angle			= 120;
	const rotation_mul		= 6.5;
	const elevation_mul		= 2.1;
	
	servo_pos.rotation = servo_pos.rotation +servo_spd.rotation *rotation_mul;
	if (servo_pos.rotation > max_angle)
	{
		servo_pos.rotation = max_angle;
	}
	else if (servo_pos.rotation < -max_angle)
	{
		servo_pos.rotation = -max_angle;
	}
	
	servo_pos.elevation = servo_pos.elevation +servo_spd.elevation *elevation_mul;
	if (servo_pos.elevation > max_angle)
	{
		servo_pos.elevation = max_angle;
	}
	else if (servo_pos.elevation < -max_angle)
	{
		servo_pos.elevation = -max_angle;
	}
